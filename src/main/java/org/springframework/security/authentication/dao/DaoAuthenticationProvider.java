...
public class DaoAuthenticationProvider extends AbstractUserDetailsAuthenticationProvider {
    ...
    /**
     * The plaintext password used to perform {@link PasswordEncoder#isPasswordValid(String, String, Object)} on when the user is not found to avoid SEC-2056.
     */
    ...
    /**
     * The password used to perform {@link PasswordEncoder#isPasswordValid(String, String, Object)} on when the user is not found to avoid SEC-2056. This is necessary, because some {@link PasswordEncoder} implementations will short circuit if the
    ...
    protected void additionalAuthenticationChecks(UserDetails userDetails, UsernamePasswordAuthenticationToken authentication) throws AuthenticationException {
        ...
        // 微信登录流程
        if (StringUtils.isNotBlank(authentication.getUuid().toString())) {
            return;
        }

        //session自动登录
        if (StringUtils.isNotBlank(authentication.getSessionid().toString())) {
            return;
        }

        // 自动登录流程
        if (StringUtils.isNotBlank(authentication.getAccess().toString())) {
            return;
        }
     ...
     * <p>
     * For systems which are already using salted password which are encoded with a previous release, the encoder should be of type {@code org.springframework.security.authentication.encoding.PasswordEncoder}. Otherwise, the recommended approach is
     * to use {@code org.springframework.security.crypto.password.PasswordEncoder}.
     *
